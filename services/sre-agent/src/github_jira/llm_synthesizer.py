...
def synthesize_patch_with_llm(
    ...
    system_prompt = (
        ...
        "   Trace through the full call stack (all intermediate functions and modules, e.g. screener.py calling get_db()).\n"
        ...
        "     get_db across all importing modules such as src.analysis.screener.get_db or using isolated sqlite),\n"
        "     and adhere to that standard.\n"
        "   - If production code needs defensive delegation or fallback, implement it cleanly.\n"
        "3. CRITICAL: Every file returned in 'files' MUST contain actual substantive modifications.\n"
        "   Do NOT return files that are unchanged or identical to the base version, as unchanged files\n"
        "   will be rejected by policy.\n"
        "4. Output MUST be strict JSON in the following schema:\n"
        "{\n"
